-- Link Desafio:  https://www.beecrowd.com.br/judge/pt/problems/view/2999
-- Link Resposta: https://www.beecrowd.com.br/judge/pt/runs/code/37438064

-- Resposta
WITH consolidado AS (
    WITH salarios_brutos AS (
        SELECT emp_venc.matr, SUM(vencimento.valor) AS valor
        FROM vencimento
        RIGHT JOIN emp_venc ON emp_venc.cod_venc = vencimento.cod_venc
        GROUP BY emp_venc.matr
    ), descontos AS (
        SELECT emp_desc.matr, SUM(desconto.valor) AS valor
        FROM desconto
        RIGHT JOIN emp_desc ON emp_desc.cod_desc = desconto.cod_desc
        GROUP BY emp_desc.matr
    )
    SELECT empregado.matr, (COALESCE(salarios_brutos.valor, 0) - COALESCE(descontos.valor, 0)) AS salarios_liquidos
    FROM empregado
    LEFT JOIN salarios_brutos ON salarios_brutos.matr = empregado.matr
    LEFT JOIN descontos ON descontos.matr = empregado.matr
), medias AS (
    SELECT empregado.lotacao, AVG(consolidado.salarios_liquidos) AS media_departamento
    FROM empregado
    LEFT JOIN consolidado ON consolidado.matr = empregado.matr
    GROUP BY empregado.lotacao
)
SELECT empregado.nome, ROUND(consolidado.salarios_liquidos, 2) AS vencimentos
FROM empregado
LEFT JOIN consolidado ON consolidado.matr = empregado.matr
LEFT JOIN medias ON medias.lotacao = empregado.lotacao
WHERE consolidado.salarios_liquidos > medias.media_departamento
AND consolidado.salarios_liquidos >= 8000;
...
CREATE TABLE dependente (matr int, nome varchar(50), endereco varchar(50));
CREATE TABLE desconto (cod_desc int, nome varchar(50), tipo varchar(10), valor numeric);
CREATE TABLE divisao (cod_divisao int, nome varchar(50), endereco varchar(50), cod_dep int);
CREATE TABLE emp_desc (cod_desc int, matr int);
CREATE TABLE emp_venc (cod_venc int, matr int);
CREATE TABLE empregado (matr int, nome varchar(50), endereco varchar(50), data_lotacao timestamp, lotacao int,
                        gerencia_cod_dep int, lotacao_div int, gerencia_div int);
CREATE TABLE vencimento (cod_venc int, nome varchar(50), tipo varchar(10), valor numeric);
